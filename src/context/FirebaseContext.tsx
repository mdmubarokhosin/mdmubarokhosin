...
export function FirebaseProvider({ children }: { children: ReactNode }) {
  ...
  const pushData = useCallback(async (path: string, value: unknown): Promise<string | null> => {
    try {
      ...
      // Read current array to maintain proper numeric indices
      const snapshot = await get(listRef);
      let currentArray: unknown[] = [];
      if (snapshot.exists()) {
        const val = snapshot.val();
        // If Firebase returns an object with numeric keys, convert to array
        if (Array.isArray(val)) {
          currentArray = val.filter((item) => item !== null);
        } else if (typeof val === 'object' && val !== null) {
          // Convert object with numeric keys to array, filter out push() keys
          currentArray = Object.keys(val)
            .filter((key) => /^[0-9]+$/.test(key))
            .sort((a, b) => Number(a) - Number(b))
            .map((key) => val[key]);
        }
      }
      // Append new item as next numeric index
      const nextIndex = currentArray.length;
      await set(ref(db, `portfolio/${path}/${nextIndex}`), value);
  ...
  const removeData = useCallback(async (path: string) => {
    try {
      // path format: "projects/2" or "skills/3"
      const parts = path.split('/');
      const arrayPath = parts[0]; // e.g., "projects"
      const indexStr = parts[1];  // e.g., "2"
      const index = Number(indexStr);

      if (!isNaN(index) && parts.length === 2) {
        // Read the full array, remove the item, and rewrite to maintain indices
        const listRef = ref(db, `portfolio/${arrayPath}`);
        const snapshot = await get(listRef);
        if (snapshot.exists()) {
          const val = snapshot.val();
          let currentArray: unknown[] = [];
          ...
          // Remove item at index and re-index
          currentArray.splice(index, 1);
          if (currentArray.length === 0) {
            await remove(listRef);
          } else {
            await set(listRef, currentArray);
          }
        } else {
          await remove(ref(db, `portfolio/${path}`));
        }
      } else {
        await remove(ref(db, `portfolio/${path}`));
      }
